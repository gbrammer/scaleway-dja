...
def run_one_fixed_slit(row=None, clean=True, **kwargs):
    ...
    if clean & (s3_root is not None):
        ...
        dirfiles = glob.glob(f"{s3_root}_fs/*")
        if len(dirfiles) == 0:
            print(f"rmdir {s3_root}_fs")
            os.rmdir(f"{s3_root}_fs")
...
def reduce_fixed_slit_obsid(
    ...
    **kwargs,
    ...
    with_diffbkg = single_file_group_diffs(exposure_groups)
    if with_diffbkg:
        other_slit_files = []
...
def single_file_group_diffs(exposure_groups):
    """
    Do diffs of slitlet files for some cases with single dithers and the
    source put on both S200A1 and S200A2 shutters, e.g., jw09180022001
    """
    ...
    for g in exposure_groups:
        if len(exposure_groups[g]) == 1:
            gsp = g.split("_")
            this = "{} {}".format(gsp[2], gsp[-1])
            this_slit = exposure_groups[g][0].split("_")[-1]

            this_file = exposure_groups[g][0]

            this_im = pyfits.open(this_file, mode="update")
            has_diff = "DIFFBKG" in this_im[0].header
            if has_diff:
                print(this_file, "has diffbkg")
                this_im.close()

                continue

            for go in exposure_groups:
                if g == go:
                    continue

                gsp = go.split("_")
                that = "{} {}".format(gsp[2], gsp[-1])

                that_data = []

                if this == that:
                    for f in exposure_groups[go]:
                        fspl = f.split("_")
                        that_slit = fspl[-1]

                        if this_slit != that_slit:
                            that_file = "_".join(fspl[:-1] + [this_slit])

                            msg = (
                                "single_file_group_diffs: " f"{this_file} - {that_file}"
                            )
                            grizli.utils.log_comment(
                                grizli.utils.LOGFILE, msg, verbose=True
                            )

                            with pyfits.open(that_file) as im:
                                this_im["DQ"].data |= im["DQ"].data

                                this_im["SCI"].data -= im["SCI"].data

                                this_im["ERR"].data = np.sqrt(
                                    this_im["ERR"].data ** 2 + im["ERR"].data ** 2
                                )

                                for ext in ["VAR_RNOISE", "VAR_FLAT", "VAR_POISSON"]:
                                    this_im[ext].data += im[ext].data

                            this_im[0].header["DIFFBKG"] = True
                            this_im.flush()
                            WITH_DIFFBKG = True
