...
def query_tile_events(max_count=200):
    ...
    if nlock > 0:
        print(f'Unlock {nlock} locked subtiles.')
        reset_locked()
        print(tlock[:10])
    
    else:
        print('No locked tiles.')
    ...
    tiles = db.SQL(f"""SELECT tile, subx, suby, filter, count(filter) as count, max(instrume) as instrume,
    ...
    print(f'Tiles to run: {len(tiles)}')
...
def run_all_tiles(count=None):
    ...
    def tile_handler(event, context, clean_flt=True, skip_existing=False, clean_result=True):
    ...
    for j in so:
        ...
        if (events[j]['exposure_count'] > break_threshold) & (1):
            print(events[j]['exposure_count'], events[j]['filter'])
            if break_max:
                break
            else:
                continue
            ...
            res = tile_handler(
                events[j], {}, skip_existing=True, clean_flt=False
            )
