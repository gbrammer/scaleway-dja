...
INIT_VARS="-var instance_count=1 -var instance_type=DEV1-L -var server_image=processor-build0 -var name_prefix=worker -var volume_size=16"
terraform plan $INIT_VARS
terraform apply $INIT_VARS -auto-approve

#### Connect to instance
SCWDOCKER=`terraform output | grep address | head -1 | awk '{print $3}' | sed "s/\"//g"`
#### grep -v $SCWDOCKER ~/.ssh/known_hosts > /tmp/hosts; mv /tmp/hosts ~/.ssh/known_hosts
